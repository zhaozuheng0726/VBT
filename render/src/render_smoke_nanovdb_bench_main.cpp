...
{
    std::cout
        << "Usage: vbt_smoke_nanovdb_bench --probe-summary <summary.json> --ray-bin <rays.bin>\n"
        << "                               --nvdb <grid.nvdb>\n"
        << "                               [--density-scale 1.0]\n"
        << "                               [--compare-tol 1e-3]\n"
        << "                               [--output-summary nanovdb_summary.json]\n";
}
...
bool parseArgs(int argc, char** argv, Options& opt)
{
    ...
    if (opt.probeSummaryPath.empty() || opt.rayBinPath.empty() || opt.nvdbPath.empty()) {
        printUsage();
        return false;
    }
    return true;
                  ...
                  float maxAbsDiff)
{
    ...
    if (!out) {
        throw std::runtime_error("Failed to open NanoVDB smoke summary output: " + path.string());
    }
    ...
    out << "  \"nvdb\": \"" << jsonEscape(opt.nvdbPath.string()) << "\",\n";
    out << "  \"frame\": " << probe.frame << ",\n";
    ...
    out << "  \"grid_byte_offset\": " << gridByteOffset << ",\n";
    out << "  \"density_scale\": " << opt.densityScale << ",\n";
    out << "  \"cpu_reference_ms\": " << cpuMs << ",\n";
    out << "  \"resident_upload_ms\": " << uploadMs << ",\n";
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        auto handle = nanovdb::io::readGrid<nanovdb::HostBuffer>(opt.nvdbPath.string(), 0, 0);
        const auto* grid = handle.grid<float>();
        if (!grid) {
            throw std::runtime_error("NanoVDB grid is null or not float: " + opt.nvdbPath.string());
        }
        const uint64_t residentBytes = handle.bufferSize();
        ...
        if (!bufferBase || !gridBase || gridBase < bufferBase) {
            throw std::runtime_error("Invalid NanoVDB buffer pointers");
        }
        ...
        appInfo.pApplicationName = "vbt_smoke_nanovdb_bench";
        ...
        const VkDeviceSize nanoBytes = static_cast<VkDeviceSize>(residentBytes);
        ...
        std::memcpy(mapped, handle.data(), static_cast<size_t>(nanoBytes));
        ...
        copyBuffer(device, queue, commandPool, stagingNanoBuffer.buffer, nanoBuffer.buffer, nanoBytes);
        ...
        push.gridByteOffset = gridByteOffset;
        ...
        std::cout << "NanoVDB smoke GPU benchmark\n";
        std::cout << "  frame: " << probe.frame << "\n";
        ...
        std::cout << "  residentUploadMs: " << uploadMs << "\n";
        ...
        std::cerr << "NanoVDB smoke benchmark failed: " << ex.what() << "\n";
