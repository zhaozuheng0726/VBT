...
{
    std::cout
        << "Usage: vbt_smoke_dense_sequence_bench --probe-summary <summary.json> --ray-bin <rays.bin>\n"
        << "                                      --raw <density.raw> --metadata <meta.json>\n"
        << "                                      [--frame-start 96] [--frame-count 8] [--ring-slots 3]\n"
        << "                                      [--density-scale 1.0] [--compare-tol 1e-3]\n"
        << "                                      [--output-summary dense_seq.json]\n";
}
...
bool parseArgs(int argc, char** argv, Options& opt)
{
    ...
    if (opt.frameCount == 0 || opt.ringSlots == 0) {
        std::cerr << "frame-count and ring-slots must be > 0\n";
        return false;
    }
    return true;
}
...
void loadDenseFrameInto(std::ifstream& in,
                        ...
                        std::vector<float>& frame)
{
    ...
    frame.resize(voxelCount);
    in.seekg(static_cast<std::streamoff>(offset), std::ios::beg);
    if (!in) {
        throw std::runtime_error("Failed to seek raw frame");
    }
    in.read(reinterpret_cast<char*>(frame.data()), static_cast<std::streamsize>(frameBytes));
    if (!in) {
        throw std::runtime_error("Failed to read raw frame");
    }
}
                  ...
                  const std::vector<double>& uploadMs)
{
    ...
    if (!out) {
        throw std::runtime_error("Failed to open sequence summary output: " + path.string());
    }
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        if (opt.frameStart < 0 || opt.frameStart >= meta.frames) {
            throw std::runtime_error("frame-start out of range");
        }
        if (opt.frameStart + static_cast<int>(opt.frameCount) > meta.frames) {
            throw std::runtime_error("frame range exceeds available frames");
        }
        opt.ringSlots = std::min<uint32_t>(opt.ringSlots, opt.frameCount);
        ...
        const size_t frameVoxelCount = vbt::frameVoxelCount(meta);
        const VkDeviceSize frameBytes = static_cast<VkDeviceSize>(frameVoxelCount * sizeof(float));
        std::vector<float> frameScratch;

        VkApplicationInfo appInfo{VK_STRUCTURE_TYPE_APPLICATION_INFO};
        appInfo.pApplicationName = "Smoke Dense Sequence Bench";
        ...
        createBuffer(physicalDevice, device,
                     ...
                     rayBuffer);
        createBuffer(physicalDevice, device,
                     static_cast<VkDeviceSize>(opt.ringSlots) * frameBytes,
                     VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                     denseBuffer);
        createBuffer(physicalDevice, device,
        ...
        for (uint32_t slot = 0; slot < preloadCount; ++slot) {
            ...
            loadDenseFrameInto(rawFile, meta, frameIndex, frameScratch);
            vkMapMemory(device, stagingDenseBuffer.memory, 0, stagingDenseBuffer.size, 0, &mapped);
            std::memcpy(mapped, frameScratch.data(), static_cast<size_t>(frameBytes));
            vkUnmapMemory(device, stagingDenseBuffer.memory);
            copyBufferRegion(device, queue, commandPool,
                             stagingDenseBuffer.buffer,
                             denseBuffer.buffer,
                             0,
                             static_cast<VkDeviceSize>(slot) * frameBytes,
                             frameBytes);
            const auto upEnd = std::chrono::high_resolution_clock::now();
            uploadMs.push_back(std::chrono::duration<double, std::milli>(upEnd - upStart).count());
        ...
        for (uint32_t i = 0; i < opt.frameCount; ++i) {
            ...
            if (slotToFrame[activeSlot] != currentFrame) {
                throw std::runtime_error("Ring slot mapping corrupted before dispatch");
            }
            ...
            push.slotStrideFloats = static_cast<uint32_t>(frameVoxelCount);
            push.activeSlot = activeSlot;
            ...
            if (!validated) {
                loadDenseFrameInto(rawFile, meta, currentFrame, frameScratch);
                const auto cpuRefStart = std::chrono::high_resolution_clock::now();
                const std::vector<float> cpuReference =
                    runCpuReference(rays, frameScratch, meta, probe.stepCount, opt.densityScale);
            ...
            const int nextFrame = opt.frameStart + static_cast<int>(i + opt.ringSlots);
            if (nextFrame < opt.frameStart + static_cast<int>(opt.frameCount)) {
                const auto upStart = std::chrono::high_resolution_clock::now();
                loadDenseFrameInto(rawFile, meta, nextFrame, frameScratch);
                vkMapMemory(device, stagingDenseBuffer.memory, 0, stagingDenseBuffer.size, 0, &mapped);
                std::memcpy(mapped, frameScratch.data(), static_cast<size_t>(frameBytes));
                vkUnmapMemory(device, stagingDenseBuffer.memory);
                copyBufferRegion(device, queue, commandPool,
                                 ...
                                 denseBuffer.buffer,
                                 0,
                                 static_cast<VkDeviceSize>(activeSlot) * frameBytes,
                                 frameBytes);
                const auto upEnd = std::chrono::high_resolution_clock::now();
                uploadMs.push_back(std::chrono::duration<double, std::milli>(upEnd - upStart).count());
        ...
        std::cout << "Dense smoke sequence benchmark\n";
        ...
        std::cerr << "Dense smoke sequence benchmark failed: " << ex.what() << "\n";
