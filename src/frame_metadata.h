...
inline std::array<int, 3> extractIntArray3(const std::string& text, const std::string& key)
{
    ...
    std::smatch m;
    if (!std::regex_search(text, m, re)) throw std::runtime_error("Missing array field: " + key);
    return {std::stoi(m[1].str()), std::stoi(m[2].str()), std::stoi(m[3].str())};
}

inline std::array<int, 3> extractIntArray3OrDefault(const std::string& text,
                                                    const std::string& key,
                                                    const std::array<int, 3>& fallback = {0, 0, 0})
{
    const std::regex re("\"" + key + "\"\\s*:\\s*\\[\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\]");
    std::smatch m;
...
inline std::array<int, 4> extractIntArray4OrDefault(const std::string& text,
                                                    const std::string& key,
...
inline FrameMetadata loadFrameMetadata(const std::filesystem::path& path)
{
    ...
    meta.bboxMin = extractIntArray3OrDefault(text, "bbox_min", {0, 0, 0});
    meta.bboxMax = extractIntArray3OrDefault(text, "bbox_max",
    ...
    meta.conversionMode = extractStringField(text, "conversion_mode");
    meta.sourceType = extractStringField(text, "source_type");
    meta.shellWidthVoxels = extractFloatField(text, "shell_width_voxels", 0.0f);
